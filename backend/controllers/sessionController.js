...
const { sendNewMeetingScheduledNotification, sendNotification, sendNotificationForFeedbackRequest, sendNotificationForSessionCancellation, sendSessionRequestNotification, sendSessionAcceptedNotification } = require('./notificationController');
...
const sendSessionRequest = async (req, res) => {
  ...
  try {
    ...
    await sendSessionRequestNotification(userId1, userId2, skill, newSession._id);
...
const acceptSessionRequest = async (req, res) => {
  ...
  try {
    ...
    // Notify the requester (userId1) that the acceptor (userId2) has accepted the session
    await sendSessionAcceptedNotification(session.userId2, session.userId1, session.skill, session._id);
...
const rejectSessionRequest = async (req, res) => {
  ...
  try {
    ...
    if (session.userId2.toString() !== req.user.id) {
      return res.status(400).json({ msg: 'You are not authorized to reject this session' });
    }

    session.status = 'canceled'; // Change status to canceled
    await session.save();

    res.json({ msg: 'Session request rejected', session });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
